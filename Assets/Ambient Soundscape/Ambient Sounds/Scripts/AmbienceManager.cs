...
namespace AmbientSounds
{
    ...
    public class AmbienceManager : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (m_autoMoveManager && GetComponent<AudioListener>() == null)
            {
                AudioListener al = FindObjectOfType<AudioListener>();
                if (al)
                {
                    Debug.LogWarning("No AudioListener found on this object at startup. Moving to Object '" + al.name + "'.", al);
                    Move(al);
                    return;
                }
                else
                {
                    Debug.LogWarning("No AudioListener found on this or any active object at startup. Searching for AudioListener...", this);
        ...
        private void OnDisable() {
            //we were just disabled. check to see if our AudioListener was disabled as well and move to a new one if it was
            AudioListener al = GetComponent<AudioListener>();
            if (m_autoMoveManager && (al == null || !al.isActiveAndEnabled)) {
        ...
        private void Update()
        {
            ...
            float now = Time.time;
            for (int i = 0; i < sequenceFades.Count; ++i) {
                ...
                if (fadeAmt > 0) {
                    foreach (Sequence data in ps.m_sequences) {
                        if (data == null)
                            continue;
                        bool found = false;
                        for (int s = 0; s < sequenceFades.Count; ++s) {
                            if (sequenceFades[s].sequence == data) {
                                if (sequenceFades[s].fade < fadeAmt) {
                                    sequenceFades[s].fade = fadeAmt;
                                    sequenceFades[s].area = ps;
                                }
                                sequenceFades[s].used = true;
                                found = true;
                                break;
                            }
                        }
            ...
            foreach (Sequence data in addedSequences) {
                if (data != null) {
                    ...
                    for (int s = 0; s < sequenceFades.Count; ++s) {
                        if (sequenceFades[s].sequence == data) {
                            if (sequenceFades[s].fade < 1) {
                                sequenceFades[s].fade = 1;
                                sequenceFades[s].area = null;
                            }
                            sequenceFades[s].used = true;
                            found = true;
                            break;
            ...
            for (int s = 0; s < fadeOutToRemoveSequences.Count; ++s)
            {
                if (fadeOutToRemoveSequences[s] != null)
                {
                    bool found = false;
                    for (int f = 0; f < sequenceFades.Count; ++f) {
                        if (sequenceFades[f].sequence == fadeOutToRemoveSequences[s]) {
                            found = true;
                            break;
                        }
                    }
        ...
        /// <param name="al">Audio Listener to move to</param>
        public static void MoveTo(AudioListener al) {
            if (_instance == null || al.gameObject == _instance.gameObject)
                return; //no move required
            _instance.Move(al);
        }
        #endregion
